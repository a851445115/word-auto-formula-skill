...
def _read_codex_openai_base_url() -> Optional[str]:
...
def _read_codex_openai_api_key() -> Optional[str]:
...
# Default to the local Codex OpenAI-compatible endpoint/model setup.
DEFAULT_CODEX_OPENAI_BASE_URL = _normalize_openai_base_url(
    os.environ.get("PDF_EXTRACTOR_OPENAI_BASE_URL")
    or _read_codex_openai_base_url()
    or "https://api.openai.com/v1"
) or "https://api.openai.com/v1"

DEFAULT_CODEX_OPENAI_API_KEY = (
    os.environ.get("PDF_EXTRACTOR_OPENAI_API_KEY")
    or os.environ.get("OPENAI_API_KEY")
    or _read_codex_openai_api_key()
    or os.environ.get("VOLCENGINE_CODING_PLAN_API_KEY")
    or os.environ.get("ARK_API_KEY")
    or ""
)

DEFAULT_CODEX_OPENAI_MODEL = os.environ.get("PDF_EXTRACTOR_OPENAI_MODEL") or "gpt-5.2"
...
def main() -> None:
    ...
    llm.add_argument(
        ...
        action="store_true",
        help=(
            "Shortcut: use current Codex OpenAI-compatible config "
            f"({DEFAULT_CODEX_OPENAI_BASE_URL}), model={DEFAULT_CODEX_OPENAI_MODEL}"
        ),
    )
    llm.add_argument(
        "--ark-code-latest",
        action="store_true",
        help="Deprecated alias of --codex-gpt-5-2 (kept for backward compatibility)",
    )
    ...
    if args.codex_gpt_5_2 or args.ark_code_latest:
        use_llm = True
        # Use our robust OpenAI-compatible JSON-only service.
        llm_service = "marker_openai_compat_service.OpenAICompatJSONService"
        openai_base_url = DEFAULT_CODEX_OPENAI_BASE_URL
        openai_api_key = DEFAULT_CODEX_OPENAI_API_KEY
        openai_model = DEFAULT_CODEX_OPENAI_MODEL
        if args.ark_code_latest and not args.codex_gpt_5_2:
            print(
                "Note: --ark-code-latest is deprecated. Using Codex config with model gpt-5.2."
            )

    if use_llm:
        # In plain --use-llm mode, fall back to current Codex defaults when omitted.
        llm_service = llm_service or "marker_openai_compat_service.OpenAICompatJSONService"
        openai_base_url = openai_base_url or DEFAULT_CODEX_OPENAI_BASE_URL
        openai_api_key = openai_api_key or DEFAULT_CODEX_OPENAI_API_KEY
        openai_model = openai_model or DEFAULT_CODEX_OPENAI_MODEL
